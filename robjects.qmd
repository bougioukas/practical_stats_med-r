...
In R, the recommended way to assign a value to an object is by using the left-arrow assignment operator (`<-`), which combines the less-than sign (`<`) and the hyphen (`-`) (keyboard shortcut:
`r kableExtra::text_spec("Alt", color = "#8676F8", bold = TRUE)` + `r kableExtra::text_spec("-", color = "#8676F8", bold = TRUE)` for Windows/Linux and
`r kableExtra::text_spec("Option", color = "#8676F8", bold = TRUE)` + `r kableExtra::text_spec("-", color = "#8676F8", bold = TRUE)` for Mac).
