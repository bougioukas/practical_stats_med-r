...
ggplot(BirthWeight, aes(height, weight)) +
  ...
  annotate("text", x = 59, y = 4920, label = "expected data point", color = "#EC7014", size = 5.5) +
...
Let's describe the underlying association between $y_i$ and $x_i$ by:
...
In a matrix notation, the outcome $Y$ is modeled by:

$$Y = X\hat{B} + e$$ {#eq-mtrx1}

Specifically, given $n$ observations of the explanatory variable X and the outcome variable Y, the @eq-mtrx1 becomes:
...
ggplot(BirthWeight, aes(x = height, y = weight)) +
  ...
  geom_hline(yintercept = 0, color = "black", linetype = "solid") +
  geom_vline(xintercept = 0, color = "black", linetype = "solid") +
  annotate("text", x = 19.0, y = -1000, label = bquote(paste (widehat(weight), "= -5412.14 + 178.31*height")), color = "blue", angle = 30.2, size = 5) +
  theme_prism(base_size = 14, base_line_size = 0.4, palette = "office") +
  labs(x = "X = Height (cm)", y = "Y = Weight (g)")
```
