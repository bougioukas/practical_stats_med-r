...
$$ P(A) =  \frac{\textrm{number of times A occured}}{\textrm{total number of experiments}}$$ {#eq-probability}
...
The conditional probability is indicated as **P(A\|B)** or A **given** B. The following formula defines the conditional probability:
...
If the knowledge of occurrence of event A does not **influence** the occurrence of another event B, the two events are called independent. Consider, for example, rolling two dice consecutively: the outcome of first die is independent of the outcome of other.

On the other hand, let's say we are interested in the following pair of events: the first is the outcome of the roll of die 1 and the second is the sum of outcomes of rolling die 1 and die 2. In this case, the outcome of the second event---i.e. the sum of both dice---depends on the first roll and the two events are not independent.
