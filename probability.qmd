...
1.  the **ample space**, Ω, which comprises all possible outcomes.
...
Let's consider the events A = "the number rolled is even" and C = "the number rolled is odd". The events A and C are mutually exclusive\index{mutually exclusive events} (also known as incompatible\index{incompatible events} or disjoint\index{disjoint events}) if they **cannot** occur simultaneously. This means that they do not share any outcomes and $A∩C =∅$.
...
The experimental probability\index{experimental probability} is based on data from repetitions of the same experiment. According to this approach, the probability of an event A, denoted by P(A), is the **relative frequency** of occurrence of the event over a total number of experiments:
...
Yet, this definition seems less clear, as it does not specify the exact interpretation of "repetitions of the same experiment" [@finetti2008].
