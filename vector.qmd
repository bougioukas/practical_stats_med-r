...
**3. Double one-element vector:** Doubles[^vector-1] can be specified in decimal (e.g., 0.000017) or in scientific (e.g, 1.7e-5) format:

[^vector-1]: Double format is a **computer number format**, usually occupying 64 bits in computer memory.
...
**4. Character one-element vector:** One-element vectors can also be characters (also known as `strings`). In R, we denote characters using single `''` or double `""` quotation marks[^vector-2]. Here, we present some examples of character one-element vectors:

[^vector-2]: Internally R stores every string within double quotes, even we have created them with single quotation marks.
