...
## Null hypothesis and alternative hypothesis for the research question
...
We will perform a pooled variance t-test (Student's t-test) to test the null hypothesis that the mean HDRS score is the same for both groups (paroxetine and placebo). Under this $H_o$, the test statistic is:
...
Under the null hypothesis, the t statistic follows the t-distribution with $n_1 + n_2 - 2$ degrees of freedom (d.f.).


::: {.callout-note}

The Student t-test does not have any restrictions on $n_1$ and $n_2$ ---**they can be equal or unequal**. However, equal samples are preferred because this maximizes the power to detect a specified difference.
:::
...
$$ 
95\% \ CI = \bar x_1 - \bar x_2 \pm t_{n-2;a/2} * SE_{\bar x_1 - \bar x_2}
$$
...
```{r}
lower_CI <- mean_dif + qt(0.025, df = 74)*st_error
lower_CI

upper_CI <- mean_dif - qt(0.025, df = 74)*st_error
upper_CI
```
...
The difference between means (20.33 - 21.49) equals to -1.16 units is not significant (failed to reject $H_0$; p = 0.16 > 0.05).  
