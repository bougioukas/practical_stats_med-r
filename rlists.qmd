...
The data frame can be created using the **`data.frame()`** function in base R, the **`tibble()`** function in the ***tibble*** package, the [`data.table()`](https://rdatatable.gitlab.io/data.table/reference/data.table.html) function in the ***data.table*** package, or the [`tidytable()`](https://markfairbanks.github.io/tidytable/) function in the ***tidytable*** package. Let's try the **`tibble()`** :
...
dat$bmi2 <- recode_values(dat$bmi, 
                      1 ~ "underweight",
                      2 ~ "healthy",
                      c(3, 4) ~ "overweight")
```

In the code above, the **`recode_values()`** function from the ***dplyr*** package matches the values on the left side of the tilde (\~) against the input vector `dat$bmi`. For example, if the values 3 or 4 are found in the `bmi` column, they are recoded as "overweight" in the new variable.
