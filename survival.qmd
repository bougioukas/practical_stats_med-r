...
The log-rank test yields a chi-square statistical value, as it is built upon this framework. Therefore, the log-rank estimates the expected number of deaths among patients and compares it with the actual number of observed deaths. The resulting p-value is 0.0003, which means that we reject the null and that the survival curves are significantly different.



### The Hazard Ratio (HR)

Proportional hazards assumption says that the ratio of hazards between groups is constant over time

The log-rank test has optimum power under the assumption of proportional hazard rates.


The Hazard ratio (HR) can be considered as an estimate of relative risk (RR), which is the risk of an event (or of developing a disease) relative to exposure. 
