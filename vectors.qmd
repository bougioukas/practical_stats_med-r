...
It's often useful to select a single element, or a set of specific elements from a vector. In the following examples, we use the built-in `month.name` vector:
...
### Select elements by indexing position within square brackets

**A. Select specific elements of a vector**

We can select parts of the vector with square brackets `[ ]` (also called extraction operator). For example:
...
So, in the second code example, the vector `3:5` created the sequence of indices `3, 4, 5` which passed to the extraction operator `[ ]`.
...
### Select elements using boolean indices (TRUE/FALSE) within square brackets

We can also pass a *logical vector* to the `[ ]` operator indicating with `TRUE` the indices we want to select and `FALSE` all others. For example, let's say that we want to select only the first four months of the year:
