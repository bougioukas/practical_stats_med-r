...
-   Explain the difference between sample and sampling distributions
-   Distinguish the point estimation from interval estimation
...
The difference between the point estimate and the population parameter is called the **error** in the estimate. Error consists of two aspects:
...
![The collection of sample means derived from repeated sampling forms a sampling distribution of the mean BP with its own characteristics.](images/many_samples.png){#fig-many_samples fig-align="center" width="70%"}
...
The standard deviation of the sampling distribution is known as the standard error (SE). There are multiple formulas for standard error depending on what is our sampling distribution. The standard error of the mean is the population σ divided by the square root of the sample size n:
...
The **Standard Error of the mean** (SEM) is a metric that describes the variability of sample means within the **sampling distribution**. It provides insight into the uncertainty associated with estimating the population mean when working with a sample, particularly when the sample size is small.
...
Let's consider sampling 5 individuals from the population and calculating their sample mean BP, denoted as $\bar{x_1}$. Next, let's repeat this process by collecting a second sample of 5 individuals and calculating the sample mean again, which we might denote as $\bar{x_2}$. This process can be iterated 100 times ($N = 100$) and generate the histogram of the sample means, $\bar{x}$ (@fig-simulation2 a). Next, we run this simulation a bunch of times with different sample size 10, 30, 50, 100 (@fig-simulation2 b, c, d, e).
...
-   As the sample size increases, the mean of a sufficiently number of sample means converges to the population mean. This property is known as the **law of large numbers**.
...
We observe that the distribution of sample means for sample size of five exhibits considerable variability (@fig-simulation a, b, c). As the sample size is increased to 30, the distributions become increasingly symmetric and tend toward approximate normality.
