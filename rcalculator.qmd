...
In R, `NULL` is distinct from both `NA` and `NaN`. It represents the **null value** and is typically used in functions to indicate that an optional argument has not been assigned a value or that a function does not produce a meaningful result (see @sec-rfunctions for more on functions).
