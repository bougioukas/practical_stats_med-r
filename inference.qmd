...
The **sample distribution** represents the probability distribution of the observed values in a randomly selected sample from the population. Using the data from this sample, we can calculate sample statistics such as the sample mean. For example, a hospital is interested in finding out the average blood pressure (BP) of its diabetic patients, but measuring each patient is impractical. Instead, they randomly selected *n* patients from this group and measured their BP. The resulting average BP for this sample, let's say $\bar{x_1} = 130$ mmHg, represents one sample mean .

![The estimate of mean, $\bar{x_1} =130$, represents an observed sample mean from an already collected sample.](images/sample1.png){#fig-sample_1 fig-align="center" width="30%"}

Consider repeating the sampling process by randomly selecting various samples, each consisting of *n* patients, and calculating their average blood pressure. This would yield a range of different sample means, such as $\bar{x_1} = 130$ mmHg, $\bar{x_2} = 128$ mmHg, $\bar{x_3} = 133$ mmHg, and so forth. This collection of multiple sample means (obtained from different samples) is the sampling distribution of the mean BP.

![The collection of sample means derived from repeated sampling forms a sampling distribution of mean with its own characteristics.](images/many_samples.png){#fig-sample_distr fig-align="center" width="75%"}
