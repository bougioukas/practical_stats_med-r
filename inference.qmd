...
In our example, the null hypothesis $H_{0}$ is that the mean serum creatinine in female patients with diabetes, $μ$, is the same as the mean serum creatinine in healthy adult women, $μ_{0}$; here μ is unknown and $μ_{0}$ is known ($μ_{0} = 0.75$ mg/dl); Conversely, the alternative hypothesis $H_{1}$ suggests a difference between the mean serum creatinine levels in these two groups. Note that $H_{1}$ may have multiple options (more than, less than, or not the same as).
...
In our example, we adopt a two-sided hypothesis, implying that the mean serum creatinine in female patients with diabetes may differ from $μ_{0} = 75$ mg/dl in either direction. That is:
...
We choose a **t-test**, a statistical method utilized to assess whether the mean of a single sample differs significantly from a known population mean. This statistical test is particularly effective when dealing with small sample sizes (N \< 30) and when the population standard deviation is unknown. The main assumption underlying the t-test is that the data being analyzed follows a normal distribution.
...
At the 5% significance level, the test result provides evidence against the null hypothesis (p = 0.014 \< 0.05). The mean serum creatinine (0.96 mg/dl) in patients with diabetes is significantly higher than the serum creatinine in healthy adult women (0.75 mg/dl).
...
We observe that the value of null hypothesis, $\mu = 0.75$, is not included in the 95% confidence interval \[0.796, 1.124\]. Consequently, the conclusion aligns with that of the hypothesis testing (rejection of $H_0$).

## The influence of sample size

To explore the effect of sample size, let's consider the scenario of the previous example, but this time with only 10 patients. First, we calculate the new value of the t test statistic:
...
We observe that the 95% confidence interval is wider \[0.709, 1.21\] and includes the value of null hypothesis, $\mu = 0.75$.

 

::: content-box-green
-   Even when a difference appears substantial, a study with small sample size may lack sufficient statistical power to attribute the difference to factors other than random chance.

-   Smaller samples lead to wider confidence intervals.
:::
