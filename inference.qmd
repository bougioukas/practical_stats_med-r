...
ggplot() +
  ...
  annotate("text", x = t + 0.2, y = 0.07, 
           label = "P(T ≥ t) = p/2", size = 3, color = "red") + 
...
Hypothesis testing and confidence intervals are two sides of the same statistical coin. While closely linked, they serve distinct purposes. 
Particularly, hypothesis testing provides a statistical decision based on the p-value, whereas confidence intervals offer an interval estimate of the effect size and its associated precision.

The 95% CI for the meanusing the t-distribution is:
...
We observe that the value of null hypothesis, $\mu = 0.75$, is not included in the 95% confidence interval [0.796, 1.124].
